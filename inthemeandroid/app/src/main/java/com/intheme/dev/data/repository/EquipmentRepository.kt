...
@Singleton
class EquipmentRepository (val apiService: ApiService, private val sharePreferenceManage: SharePreferenceManage) {
    ...
    fun deleteEquipment(strId:String=""): Flow<BaseStringResponse> = flow {
        emit(apiService.deleteEquipment(id = strId))
    }.flowOn(Dispatchers.IO)


    fun getEquipmentList(queryMap: Map<String, Any> = mapOf()): Flow<BaseObjectResponse<BaseListResponse<EquipmentResponse>>> = flow {
        emit(apiService.getEquipmentList(queryMap))
    }.flowOn(Dispatchers.IO)
